...
def clean_trace_data(trace):
    ...
    return trace_post, Obs_PostBBW

def main(ids_filepath = 'IDs.csv', agg_level = 'daily'):
    ...
    if not os.path.isfile(ids_filepath):
        raise ValueError('ids_filepath must be a valid file path to CUSIP IDs')
    ...
    for i in range(0,len(cusip_chunks)):  
        ...
        if len(trace) <= 100:
            ...
            trace_post, CleaningExport['Obs.PostBBW'].iloc[i] = clean_trace_data(trace)
